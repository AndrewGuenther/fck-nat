...
| `mss_clamp`             | If set, clamps TCP MSS to the path MTU on forwarded connections (mangle-table `TCPMSS --clamp-mss-to-pmtu`). Managed NAT Gateway does this transparently; enable for parity when instances use the default jumbo-frame (9001) MTU and egress crosses a 1500-MTU internet gateway path, where a broken path-MTU discovery return leg otherwise leaves large transfers to some destinations hanging. |
